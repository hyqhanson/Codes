...
// without iteration
template <typename P>
void linearRegression(const vector<vector<P>> &x,
                      const vector<P> &y,
                      P &slope,
                      P &intercept)
{
  ...
  if (denominator != 0)
  {
    ...
    intercept = (double)Sy / n - (double)slope * ((double)Sx / n);
  }
  else
  {
...
template <typename P>
void printRecord(const vector<vector<P>> &data)
{
...
template <typename P, typename T>
void printResultPosit(vector<vector<double>> Records,
                      string filename,
                      int size,
                      int es,
                      int models,
                      ...
                      int numIterations,
                      unsigned seed,
                      int scaled, vector<posit128_4> &stored_result)
{
  vector<vector<P>> Records_posit;
  ...
  // Scaling
  vector<vector<P>> scaled_Records(Records.size(),
                                   vector<P>(Records[0].size(), 0.0));
  vector<vector<P>> scalingFactor;
  if (scaled == 1)
  {
    cout << "Using min-max" << endl;
    minMaxScale2D(Records_posit, scalingFactor);
  }
  else if (scaled == 2)
  {
    cout << "Using standard" << endl;
    standard2D(Records_posit, scalingFactor);
  }
  ...
  if (models == 1)
  {
    ...
    if (scaled != 0)
    {
      ...
      if (scaled == 1)
      {
        for (size_t i = 0; i < x.size(); ++i)
        {
          ...
          posit128_4 error = (posit128_4)result[i] - (posit128_4)Records[i][0];
    ...
    if (scaled == 0)
    {
      ...
      stored_result.push_back(mse);
      cout << model.hypothesis(x[0]) << " " << Records[0][0] << " " << model.hypothesis(x[1]) << " " << Records[1][0] << endl;
    }
  ...
  else if (models == 2)
  {
    ...
    for (const auto &row : testingSet)
    {
      ...
      int resultLabels = logistic_model.roundProbability(prediction, 0.7);
...
template <typename P, typename T>
void printResultIEEE(vector<vector<double>> Records,
                     int models,
                     double learningRate,
                     int numIterations,
                     unsigned seed,
                     int scaled)
{
  ...
  {
    cout << "Using min-max" << endl;
    minMaxScale2D(Records_IEEE, scalingFactor);
  }
  else if (scaled == 2)
  {
    cout << "Using standard" << endl;
    standard2D(Records_IEEE, scalingFactor);
  }
...
int main(int argc, char **argv)
{
  ...
  // Store all result from Posit (relative error etc.) for comparsion
  vector<posit128_4> stored_result;
...
#define noScale(size, r)                                            \
  ...
  // Find out the best solution of posits
  int pos = 0;
  posit128_4 best_result = std::numeric_limits<double>::max();
  for (size_t i = 0; i < stored_result.size(); i++)
  {
    if (stored_result[i] < best_result)
    {
      best_result = stored_result[i];
      pos = i; // Update pos to the current index
    }
  }

  cout << "Best solution at es " << pos << " has " << best_result << endl;

  cout << "====================================" << endl;
  cout << "half vs Double: " << endl;
  printResultIEEE<half, double>(
      Records, models, learningRate, numIterations, seed, scale);
